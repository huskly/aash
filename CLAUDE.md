...
- Pre-rescue withdrawal amount is the wallet shortfall needed to reach `targetHF` plus a hardcoded 500 debt-token buffer, with a hardcoded 500 debt-token execution minimum to avoid gas-inefficient dust withdrawals. It remains capped by `maxVaultWithdrawAmount`, user-specific vault capacity, and layer-1 `maxRepayAmount` during HF capacity planning.
